...
$$
\text{mangled}\_\text{address} = \texttt{rol}(\text{address} \oplus\ \text{key} , \text{0x11})
$$
$$
\text{address} = \texttt{ror}(\text{mangled}\_\text{address}, \text{0x11})\ \oplus\ \text{key}
$$
...
$$
\text{key} = \texttt{ror}(\text{mangled}\_\text{address}, \text{0x11})\ \oplus \ \text{address}
$$
...
Now we have everything to get the key, by overwriting the struct with the mangled `system()` address and `/bin/sh` as argument we simply need to run the program and exit to get a shell.
